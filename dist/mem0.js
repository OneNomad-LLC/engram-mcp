...
async function getMem0Client(apiKey) {
 ...
 */
export async function mem0Extract(config, storage, messages, conversationId) {
    const apiKey = config.mem0ApiKey || process.env.MEM0_API_KEY;
    if (!apiKey)
        throw new Error('MEM0_API_KEY is required for Mem0 extraction');
    ...
    for (const mem of mem0Results) {
        ...
        if (!content || existingContents.has(content.toLowerCase().trim()))
            continue;
        const chunk = {
            id: randomUUID(),
            tier: 'short-term',
 ...
 */
export async function mem0Search(config, query, limit = 10) {
    const apiKey = config.mem0ApiKey || process.env.MEM0_API_KEY;
    if (!apiKey)
        return [];
    const client = await getMem0Client(apiKey);
 ...
 */
export async function mem0SyncAll(config, storage) {
    const apiKey = config.mem0ApiKey || process.env.MEM0_API_KEY;
    if (!apiKey)
        throw new Error('MEM0_API_KEY is required for Mem0 sync');
    const client = await getMem0Client(apiKey);
    const userId = config.mem0UserId || 'default';
...
function classifyCognitiveLayer(content) {
    const lower = content.toLowerCase();
