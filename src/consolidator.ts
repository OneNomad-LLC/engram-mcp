 ...
 */
function getActiveExemplars(chunks: StoredChunk[], windowDays: number = 7): StoredChunk[] {
  const now = Date.now();
  const cutoff = now - windowDays * 86_400_000;

  return chunks
    .filter(c => {
      if (c.tier === 'archive') return false;
...
function computeProximityFactor(
  ...
  if (chunk.embedding && chunk.embedding.length > 0) {
    ...
    for (const ex of exemplars) {
      if (!ex.embedding || ex.embedding.length !== chunk.embedding.length) continue;
  ...
  let maxJaccard = 0;
  for (const ex of exemplars) {
    if (ex.tags.length === 0) continue;
    const exTags = new Set(ex.tags);
    let intersection = 0;
    for (const t of chunkTags) if (exTags.has(t)) intersection++;
...
async function selfOrganize(storage: Storage, chunks: StoredChunk[]): Promise<number> {
  let organized = 0;

  // Phase 1: Auto-describe — infer domain/topic from content heuristics
  const undescribed = chunks.filter(c =>
    c.tier !== 'archive' && (!c.domain || !c.topic)
  );

  for (const chunk of undescribed.slice(0, 50)) {
    const updates: Partial<StoredChunk> = {};
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    for (let j = i + 1; j < candidates.length; j++) {
      ...
      if (existingTargets.has(b.id)) continue;
