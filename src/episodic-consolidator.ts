...
export async function consolidateEpisodic(
  ...
  storage: Storage
): Promise<EpisodicConsolidationStats> {
  const stats: EpisodicConsolidationStats = { clustered: 0, summarized: 0, promoted: 0 };
  ...
  if (candidates.length < 3) return stats;
  ...
  for (const cluster of clusters) {
    ...
    const summaryChunk: StoredChunk = {
      id: randomUUID(),
      tier: 'long-term',
      content: summary,
      type: 'fact',
  ...
  for (const chunk of l1Chunks) {
    if ((chunk.consolidationLevel ?? 0) !== 1) continue;
...
function shouldCluster(seed: StoredChunk, candidate: StoredChunk, cluster: StoredChunk[]): boolean {
  ...
  if (seedTags.size > 0 && candTags.size > 0) {
    let intersection = 0;
    for (const t of seedTags) if (candTags.has(t)) intersection++;
  ...
  if (isLlmAvailable()) {
    try {
      return await llmComplete(
        config,
        'You consolidate related memories into a single concise summary. Extract the shared principle, pattern, or key fact. Output 1-2 sentences maximum. Do not add commentary.',
        `Related memories:\n- ${contents}`,
...
function extractSharedTags(cluster: StoredChunk[]): string[] {
...
function getDateRange(cluster: StoredChunk[]): string {
