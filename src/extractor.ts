...
// Sentence-level pattern matching. Different approach from MemPalace --
// we split into sentences and score each independently.
  ...
  for (const msg of messages) {
    ...
    for (const sentence of sentences) {
      for (const pattern of HEURISTIC_PATTERNS) {
        if (!pattern.test.test(sentence)) continue;
