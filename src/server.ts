...
import type { MemoryType, Sentiment } from './types.js';
import {
...
server.registerTool(
  ...
  async ({ content, type, importance, tags, source, domain, topic, sentiment, emotionalValence, emotionalArousal, skipDedupe, origin, tier, createdAt, skipKgExtraction, skipDailyEntry }) => {
    const storage = await ensureStorage();
...
server.registerTool(
  ...
  async ({ id, metadata, mode }) => {
    ...
    const effectiveMode: UpdateMetadataMode = mode ?? 'merge';
    ...
    const patch = buildUpdateMetadataPatch(metadata as UpdateMetadataInput, effectiveMode);
    ...
    process.stderr.write(
      `[engram] memory_update_metadata id=${id} mode=${effectiveMode} diff=${JSON.stringify(diff)}\n`,
    );
...
server.registerTool(
  ...
  async ({ action, content, domain }) => {
    ...
    if (action === 'check') {
      if (!content) return json({ error: 'Content required for contradiction check.' });
