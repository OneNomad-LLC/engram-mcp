 ...
 */
export function buildUpdateMetadataPatch(
  metadata: UpdateMetadataInput,
  mode: UpdateMetadataMode,
): Partial<MemoryChunk> {
  const patch: Partial<MemoryChunk> = {};
  if (mode === 'replace') {
    patch.tags = metadata.tags ?? [];
    patch.source = metadata.source ?? '';
    patch.domain = metadata.domain ?? '';
    patch.topic = metadata.topic ?? '';
    patch.type = metadata.type ?? ('context' as MemoryType);
    patch.sentiment = metadata.sentiment ?? ('neutral' as Sentiment);
    patch.importance = metadata.importance ?? 0.5;
    if (metadata.cognitiveLayer !== undefined) {
      patch.cognitiveLayer = metadata.cognitiveLayer;
    }
  } else {
    if (metadata.tags !== undefined) patch.tags = metadata.tags;
    if (metadata.source !== undefined) patch.source = metadata.source;
    if (metadata.domain !== undefined) patch.domain = metadata.domain;
    if (metadata.topic !== undefined) patch.topic = metadata.topic;
    if (metadata.type !== undefined) patch.type = metadata.type;
    if (metadata.sentiment !== undefined) patch.sentiment = metadata.sentiment;
    if (metadata.importance !== undefined) patch.importance = metadata.importance;
    if (metadata.cognitiveLayer !== undefined) {
      patch.cognitiveLayer = metadata.cognitiveLayer;
    }
  }
